...
scriptInjection("https://code.jquery.com/jquery-3.2.1.min.js", function () {
  ...
  (function (a) {
    ($jq321.browser = $jq321.browser || {}).mobile =
  ...
  function getServerUrls() {
    ...
    if (thisLibUrl === "") {
      return {
        backend:
          "https://tracking-sales-pop.carecart.io/index.php/FrontController/",
    ...
    var backend =
      "https://" + tempAnchorTag.hostname + "/index.php/FrontController/";
    if ("sales-pop.carecart.io" === tempAnchorTag.hostname) {
      backend =
        "https://tracking-" +
        tempAnchorTag.hostname +
      ...
      backend =
        "https://uat-tracking-sales-pop.carecart.io/index.php/FrontController/";
    }
    ...
    return {
      ...
      css:
        "https://" +
        tempAnchorTag.hostname +
        "/public/front_assets/new-ui/css/notif-box.css?v" +
        version,
      cssStock:
        "https://" + tempAnchorTag.hostname + "/lib/stock-box.css?v" + version,
      cssTimer:
        "https://" + tempAnchorTag.hostname + "/lib/timer-box.css?v" + version,
      cssVisitor:
        "https://" +
        tempAnchorTag.hostname +
        "/lib/visitor-box.css?v" +
        version,
      cssSold:
        "https://" + tempAnchorTag.hostname + "/lib/sold-box.css?v" + version,
      cssQuick:
        "https://" + tempAnchorTag.hostname + "/lib/quick-box.css?v" + version,
      cssTrustBadges:
        "https://" + tempAnchorTag.hostname + "/lib/badges-box.css?v" + version,
  ...
  var getNotificationsByCollectionHandles = function (collectionHandles) {
    ...
    var relevantProductHandles =
      getProductHandlesByCollectionHandles(collectionHandles);
    spDebuger.storeLog(
      "relevantProductHandles count: " + relevantProductHandles.length
    );
    ...
    if (relevantProductHandles == null || relevantProductHandles.length === 0) {
      spDebuger.storeLog(
        "No relevant product found for this collection: " + collectionHandles
      );
    ...
    Array.prototype.forEach.call(
      apiResponse.allNotifications,
      function (notification) {
        Array.prototype.forEach.call(
          relevantProductHandles,
          function (product) {
    ...
    spDebuger.storeLog(
      "relevantNotifications count: " + relevantNotifications.length
    );
  ...
  var getNotificationsByProduct = function (currentProductHandle) {
    ...
    var collectionHandles =
      getCollectionHandlesByProductHandle(currentProductHandle);
    ...
    var notificationsByCollectionHandles =
      getNotificationsByCollectionHandles(collectionHandles);

    spDebuger.storeLog(
      "notificationsByCollectionHandles count: " +
        notificationsByCollectionHandles.length
    );
  ...
  var getProductHandlesByCollectionHandles = function (collectionHandles) {
    ...
    spDebuger.storeLog(
      "uniqueRelevantProducts count: " + uniqueRelevantProducts.length
    );
  ...
  var getRelevantNotifications = function () {
    ...
    spDebuger.storeLog(
      "filteredNotifications Count: " + filteredNotifications.length
    );
  ...
  var getCollectionHandlesByProductHandle = function (currentProductHandle) {
    ...
    var collectionHandles = "";
    Array.prototype.forEach.call(
      apiResponse.allProductsWithCollections,
      function (obj) {
  ...
  var isNotificationAllowedOnCurrentPage = function () {
    ...
    if (
      $jq321.inArray("products", currentPageHandle) != -1 &&
      $jq321.inArray("collections", currentPageHandle) != -1
    ) {
    ...
    if ("products" === currentPageTypep && "collections" === currentPageTypec) {
      ...
      Array.prototype.forEach.call(
        apiResponse.restrictionSettings,
        function (page) {
          if (
            page.product_id == null &&
      ...
      // b. Check by Collection
      Array.prototype.forEach.call(
        apiResponse.restrictionProducts,
        function (obj) {
  ...
  var shouldStatsBeShown = function () {
    return typeof URLSearchParams === "undefined"
      ? false
      : new URLSearchParams(window.location.search).has("show-sp-config");
  };

  var printConfigForNerds = function () {
    var statsHtml =
      "<div id='sp-stats-container'><span>::Sales Popup Configuration::</span><ol>";
    ...
    statsHtml +=
      "<li>Hidden By Cookie: <b>" +
      (isHidePopupCookieSet() ? "Yes" : "No") +
      "</b></li>";
    statsHtml += "</ol>";
    ...
    var api_url =
      salespoplib_vars_obj.backend_url +
      "checkStore/?" +
      $jq321.param(queryStringData);

    statsHtml +=
      "<a target='_blank' href='" +
      api_url +
      "'>See all generated notifications</a></div>";
  ...
  window.checkmodule_popup = function (response) {
    ...
    try {
      if (typeof Storage !== "undefined") {
        ...
        if (
          typeof apiResponse.allCollectionsWithProducts !== "undefined" &&
          typeof apiResponse.allNotifications !== "undefined" &&
          typeof apiResponse.allProductsWithCollections !== "undefined"
        ) {
          ...
          var notificationsExistingData =
            window.localStorage.getItem("NotificationsData");
        ...
        if (dataReceived === false) {
          ...
          if (notificationsData !== null) {
            ...
            apiResponse.allCollectionsWithProducts =
              notificationsData.allCollectionsWithProducts;
            apiResponse.allNotifications = notificationsData.allNotifications;
            apiResponse.allProductsWithCollections =
              notificationsData.allProductsWithCollections;
          }
        }
      }
    } catch (msg) {
    ...
    if (
      apiResponse &&
      apiResponse.sold &&
      apiResponse.sold !== null &&
      apiResponse.sold.on_off == 1
    ) {
      $jq321("head").append(
        $jq321("<link/>", {
          rel: "stylesheet",
    ...
    if (
      apiResponse &&
      apiResponse.quickView &&
      apiResponse.quickView !== null
    ) {
      $jq321("head").append(
        $jq321("<link/>", {
          rel: "stylesheet",
      ...
      if (
        apiResponse &&
        apiResponse.quickViewCollection &&
        apiResponse.quickViewCollection == 1
      ) {
        setTimeout(function () {
          // PRODUCT QUICK VIEW FOR COLLECTION PAGES
          collectionQuickView(
            apiResponse.quickViewCollectionText,
            apiResponse.quickViewCollectionLayout,
            apiResponse.quickViewCollectionPosition
          );
        }, 3000);
      }
    }
    ...
    if (
      apiResponse &&
      apiResponse.trustBadges &&
      apiResponse.trustBadges != false
    ) {
      $jq321("head").append(
        $jq321("<link/>", {
          rel: "stylesheet",
    ...
    if (
      apiResponse.hasOwnProperty("string") &&
      (apiResponse.string === "sales_notification_disabled" ||
        apiResponse.string === "no_data")
    ) {
    ...
    if (
      apiResponse.hasOwnProperty("disable_on_cart_page") &&
      parseInt(apiResponse.disable_on_cart_page) === 1
    ) {
    ...
    if (
      apiResponse.hasOwnProperty("isLegacyStore") &&
    ...
    spDebuger.storeLog(
      "Total Relevant Notifications: " + relevantNotifications.length
    );

    var irrevantNotifications =
      apiResponse.allNotifications.filterRelevantNotifications(
    ...
    if (notificationCount < apiResponse.max_noti) {
      ...
      spDebuger.storeLog(
        "Irrelevant Notifications to be included: " + notificationDifference
      );
      spDebuger.storeLog("Irrelevant Notifications:");

      for (
        var c = 0;
        c < notificationDifference && c < irrevantNotifications.length;
        c++
      ) {
    ...
    spDebuger.storeLog(
      "Total Notifications to be shown: " + notificationsToShow.length
    );
  ...
  window.showSalesPopup = function (popUpIndexToDisplay) {
    ...
    var s =
      apiResponse.allNotifications[popUpIndexToDisplay].order_generated_time;
    var a = s.split(/[^0-9]/);
    ...
    if (
      !notificationsToShow[popUpIndexToDisplay].hasOwnProperty("notifications")
    ) {
      return false;
    }

    var dataNotification =
      notificationsToShow[popUpIndexToDisplay].notifications;
  ...
  $jq321("body").on("click", "#noti-rsn-id", function (e) {
    ...
    var GetURL =
      "https://" + encodeURIComponent(store_domain_grabber) + "/cart.json";
    var cartToken = "";
  ...
  function stockCountdown(responseStock) {
    ...
    var selectorStock3 = $jq321("form[action='/cart/add']:first")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorStock4 = $jq321("form[action='/cart/add']:first");
    var selectorStock5 = $jq321("#shopify-section-product-template")
      .find("form[action='/cart/add']")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorStock6 = $jq321("#shopify-section-product-template").find(
  ...
  function timeCountdown(responseTimer) {
    var selectorTimer1 = $jq321("form[action='/cart/add']")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorTimer2 = $jq321("form[action='/cart/add']");
    var selectorTimer3 = $jq321("form[action='/cart/add']:first")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorTimer4 = $jq321("form[action='/cart/add']:first");
    var selectorTimer5 = $jq321("#shopify-section-product-template")
      .find("form[action='/cart/add']")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorTimer6 = $jq321("#shopify-section-product-template").find(
      "form[action='/cart/add']"
    );
  ...
  function visitorCounter(response) {
    var selectorVisitor1 = $jq321("form[action='/cart/add']")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorVisitor2 = $jq321("form[action='/cart/add']");
    var selectorVisitor3 = $jq321("form[action='/cart/add']:first")
      .find("button[type='submit'],input[type='submit']")
      .parent();
  ...
  function soldCounter(response) {
    var selectorSold1 = $jq321("form[action='/cart/add']")
      .find("button[type='submit'],input[type='submit']")
      .parent();
    var selectorSold2 = $jq321("form[action='/cart/add']");
    var selectorSold3 = $jq321("form[action='/cart/add']:first")
      .find("button[type='submit'],input[type='submit']")
      .parent();
  ...
  function collectionQuickView(
    quickViewCollectionText,
    quickViewCollectionLayout,
    quickViewCollectionPosition
  ) {
    var allLinks = [];
    var product_id = meta.product && meta.product.id ? meta.product.id : "";
    if (product_id == "") {
      var masterSelector = "a";
      if (Shopify.shop == "londonlash-us.myshopify.com") {
        masterSelector = ".container a";
      }
      $jq321(masterSelector).each(function () {
    ...
    $jq321("img").each(function () {
      ...
      console.log("data_img", data_image);
      if (
        (href !== undefined && data_image !== undefined) ||
        (srcset !== undefined && product_id == "")
      ) {
        if (quickViewCollectionLayout == 2) {
          // FOR QUICK VIEW BUTTON
          var newButton =
            '<input type="button" class="quick-view collection-quick-view" value="' +
            quickViewCollectionText +
            '" data-product-url="' +
            allLinks[linkCount] +
            '" data-quick-view="1">';
  ...
  function trustBadges(trustBadgesResponse) {
    if (trustBadgesResponse.product_page_show_hide == 1) {
      ...
      var selectorTrustBadges1 = $jq321("form[action='/cart/add']")
        .find("button[type='submit'],input[type='submit']")
        .parent();
      var selectorTrustBadges2 = $jq321("form[action='/cart/add']");
      var selectorTrustBadges3 = $jq321("form[action='/cart/add']:first")
        .find("button[type='submit'],input[type='submit']")
        .parent();
    ...
    if (
      trustBadgesResponse.badges_style == 1 ||
      trustBadgesResponse.badges_style == 3
    ) {
      $jq321("#CloneBox svg")
        .find("path.ChangeColor-preview")
        .css("fill", trustBadgesResponse.badges_color);
    }

    if (
      trustBadgesResponse.badges_style == 1 ||
      trustBadgesResponse.badges_style == 2
    ) {
      $jq321("#CloneBox")
        .find("div.BorderBox")
        .addClass("badges-style-original");
    }

    if (
      trustBadgesResponse.badges_style == 3 ||
      trustBadgesResponse.badges_style == 4
    ) {
      $jq321("#CloneBox")
        .find("div.BorderBox")
        .addClass("background-store-front");
    }
  }
  // ---------------------------------- </TRUST BADGES MODULE> --------------------------------
